...
// =========================================================
typedef enum {
    TK_FUNC, TK_RETURN, TK_IF, TK_WHILE, TK_ID, TK_NUM, // <-- 已加入 TK_WHILE
...
void next_token() {
    ...
    if (isdigit(*src)) {
        ...
        else if (strcmp(cur_token.text, "while") == 0) cur_token.type = TK_WHILE; // <-- 已加入關鍵字識別
...
void statement() {
    ...
    else if (cur_token.type == TK_WHILE) { // <-- 已加入 WHILE 語法處理與回填邏輯
        next_token(); // skip while
        next_token(); // skip (
        
        int cond_pc = quad_count; // 記錄條件檢查的起點 L1
        
        char cond[32]; expression(cond);
        next_token(); next_token(); // skip ) {
        
        int jmp_f_idx = quad_count;
        emit("JMP_F", cond, "-", "?"); // 條件不成立時跳出 L2 (暫填 ?)
        
        while (cur_token.type != TK_RBRACE) statement();
        next_token(); // skip }
        
        char start_pc_str[32];
        sprintf(start_pc_str, "%d", cond_pc);
        emit("JMP", "-", "-", start_pc_str); // 無條件跳回 L1
        
        // 兌現支票：將正確的 L2 跳出地址填回 JMP_F
...
void vm() {
    ...
    while (pc < quad_count) {
        ...
        else if (strcmp(q.op, "JMP") == 0) { // <-- 已加入無條件跳轉支援
            pc = atoi(q.result) - 1; 
        }
